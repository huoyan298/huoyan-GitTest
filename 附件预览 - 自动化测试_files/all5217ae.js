var gsAgent=navigator.userAgent.toLowerCase(),gsAppVer=navigator.appVersion.toLowerCase(),gsAppName=navigator.appName.toLowerCase(),gbIsOpera=gsAgent.indexOf("opera")>-1,gbIsWebKit=gsAgent.indexOf("applewebkit")>-1,gbIsKHTML=gsAgent.indexOf("khtml")>-1||gsAgent.indexOf("konqueror")>-1||gbIsWebKit,gbIsIE=(gsAgent.indexOf("compatible")>-1&&!gbIsOpera)||gsAgent.indexOf("msie")>-1,gbIsTT=gbIsIE?(gsAppVer.indexOf("tencenttraveler")!=-1?1:0):0,gbIsQB=gsAppVer.indexOf("qqbrowser")!==-1||gsAppVer.indexOf("tencenttraveler")!==-1,gbIsQBWebKit=gbIsWebKit?(gsAppVer.indexOf("qqbrowser")!=-1?1:0):0,gbIsQPlus=gsAgent.indexOf("qplus")>-1,gbIsSogou=gsAgent.indexOf("se 2.x metasr 1.0")>-1,gbIsChrome=gbIsWebKit&&!gbIsQBWebKit&&gsAgent.indexOf("chrome")>-1&&!gbIsSogou&&!gbIsQPlus,gbIsSafari=gbIsWebKit&&!gbIsChrome&&!gbIsSogou&&!gbIsQBWebKit,gbIsQBIE=gbIsIE&&gsAppVer.indexOf("qqbrowser")!=-1,gbIsFF=gsAgent.indexOf("gecko")>-1&&!gbIsKHTML,gbIsTrident=gsAppVer.indexOf("trident")!=-1,gbIsEdge=gsAgent.indexOf('edge')!=-1,gbIsNS=!gbIsIE&&!gbIsOpera&&!gbIsKHTML&&(gsAgent.indexOf("mozilla")==0)&&(gsAppName=="netscape"),gbIsAgentErr=!(gbIsOpera||gbIsKHTML||gbIsSafari||gbIsIE||gbIsTT||gbIsFF||gbIsNS),gbIsWin=gsAgent.indexOf("windows")>-1||gsAgent.indexOf("win32")>-1,gbIsVista=gbIsWin&&(gsAgent.indexOf("nt 6.0")>-1||gsAgent.indexOf("windows vista")>-1),gbIsWin7=gbIsWin&&gsAgent.indexOf("nt 6.1")>-1,gbIsWin10=gbIsWin&&gsAgent.indexOf("nt 10.0")>-1,gsWinVer=gbIsWin&&/windows nt (.*);/.test(gsAgent)&&(RegExp.$1+''),gbIsMac=gsAgent.indexOf("macintosh")>-1||gsAgent.indexOf("mac os x")>-1,gsMacVer=/mac os x (\d+)(\.|_)(\d+)/.test(gsAgent)&&parseFloat(RegExp.$1+"."+RegExp.$3),gbIsLinux=gsAgent.indexOf("linux")>-1,gbIsAir=gsAgent.indexOf("adobeair")>-1,gnIEVer=/MSIE (\d+.\d+);/i.test(gsAgent)&&parseFloat(RegExp["$1"]),gnIEDocTypeVer=0,gsFFVer=/firefox\/((\d|\.)+)/i.test(gsAgent)&&RegExp["$1"],gsSafariVer=""+(/version\/((\d|\.)+)/i.test(gsAgent)&&RegExp["$1"]),gsChromeVer=""+(/chrome\/((\d|\.)+)/i.test(gsAgent)&&RegExp["$1"]),gsQBVer=""+(/qqbrowser\/((\d|\.)+)/i.test(gsAgent)&&RegExp["$1"]),_gsForEBuiltTag="_For_E_Built";
(function(){
if(gbIsTrident)
{
gbIsFF=false;
...
function setCookie(e,g,c,f,d,b,a)
{
if(e)
{
document.cookie=T(['$name$=$value$; ',!c?'':'expires=$expires$; ','path=$path$; ','domain=$domain$; ',!b?'':'$secure$']).replace({name:e,value:a===false?(g||""):encodeURIComponent(g||""),expires:c&&c.toGMTString(),path:f||'/',domain:d||["mail.",getDomain()].join(""),secure:b?"secure":""});
return true;
}
else{
return false;
...
{
return a&&a.replace?(a.replace(/&lt;/gi,"<").replace(/&gt;/gi,">").replace(/&apos;/gi,"'").replace(/&$#39;/gi,"'").replace(/&amp;/gi,"&").replace(/&quot;/gi,"\"")):a;
}
...
{
return b&&b.replace(/<script><\/script>/ig,"").replace(/<script ?.*>(.*?)<\/script>/ig,"<script>$1\n</script>").replace(/<script ?.*>([\s\S]*?)<\/script>/ig,a||"");
}
...
function hasInstallMailApp()
{
var c=getTop(),b=c.goUserInfo.get("IOSWEBAPPVERNEW")||"",a=c.goUserInfo.get("ANDROIDWEBAPPVERNEW")||"";
return !!(b||a);
}
function hasInstallFixedVersion(d)
{
var c=getTop(),b=c.goUserInfo.get("IOSWEBAPPVERNEW")||"",a=c.goUserInfo.get("ANDROIDWEBAPPVERNEW")||"";
return !!(b&&(versionCompare(b,d,{zeroExtend:true})!==-1)||a&&(versionCompare(a,d,{zeroExtend:true})!==-1));
}
function hasNotInstallMailAppAndIsNotTencentUin()
{
var a=getTop();
return !!(!a.gbIsTencentUin&&!a.hasInstallFixedVersion("5.3.0"));
}
function versionCompare(e,g,d)
{
var c=d&&d.lexicographical,j=d&&d.zeroExtend,f=e.split('.'),h=g.split('.');
function b(i)
{
return (c?/^\d+[A-Za-z]*$/:/^\d+$/).test(i);
}
if(j)
{
while(f.length<h.length)
f.push("0");
while(h.length<f.length)
h.push("0");
}
...
{
try{
c(xmailIframe.contentWindow.xmlHttp());
}
catch(d)
{
c(window.ActiveXObject?new ActiveXObject('Microsoft.XMLHTTP'):(window.XMLHttpRequest?new XMLHttpRequest():null));
}
}
}
if(!xmailIframe)
...
if(!S('xmail_iframe'))
{
createIframe(getTop(),'https://wx.mail.qq.com/list/readtemplate?name=ajax_proxy.html',{id:'xmail_iframe',onload:function(){
xmailIframe=this;
b(a);
}});
}
...
function xmailAutoLogin(a,c)
{
var b;
if(isXmailSidExpired()||c)
{
loginCallbacks.push(a);
if(!loging)
{
loging=true;
doRawXmailRequest('login/exchangeticket',{enc:1,func:1,sid:getSid(),cookie_sid:getCookie('sid')},'',function(f,e){
loging=false;
if(f&&e)
{
var d=JSON.parse(e);
if(d&&d.head&&d.head.ret===0&&d.body)
{
setCookie('xm_sid',d.body.xm_sid,null,'/','mail.qq.com',true,false);
setCookie('xm_skey',d.body.xm_skey,null,'/','mail.qq.com',true,false);
setCookie('xm_uin',d.body.xm_uin,null,'/','mail.qq.com',true,false);
}
else{
b='xmailAutoLogin failed! ret: '+(d&&d.head&&d.head.ret);
}
}
else{
b='xmailAutoLogin failed! network error!';
}
E(loginCallbacks,function(g){
if(g&&typeof g==='function')
{
g(b,{xmailSid:getCookie('xm_sid')});
}
});
loginCallbacks=[];
});
...
{
a(b,{xmailSid:getCookie('xm_sid')});
}
}
}
function isXmailSidExpired()
{
return !(getCookie('xm_sid')&&getCookie('xm_skey')&&getCookie('xm_uin'));
}
function setXmailSidExpired()
{
deleteCookie('xm_sid');
deleteCookie('xm_skey');
deleteCookie('xm_uin');
}
function getXmailRandomString()
{
return parseInt(Math.random()*10000000000000)+''+Date.now();
}
function collectXmailParams(a,b,c)
{
if(typeof c==='object')
{
c=JSON.stringify(c);
...
function doRawXmailRequest(a,f,i,e,b)
{
b=b||{};
if(a.indexOf('/')===0)
{
...
{
E(f[d],function(j){
collectXmailParams(c,d,j);
});
}
else{
collectXmailParams(c,d,f[d]);
}
}
}
var g=c.join('&');
var h=/^https?:\/\//.test(a)?a:('https://wx.mail.qq.com/'+a);
if(h.indexOf('?')===-1)
{
h+='?';
}
...
if(!b.httpCache)
{
h+='&r='+getXmailRandomString();
}
createXMLHttpRequest(function(j){
QMAjax.send(h,{xhr:j,content:g,method:b.method||'POST',onload:e});
});
}
function doXmailRequest(a,d,c,b)
{
c=c||function(){
};
b=b||{};
xmailAutoLogin(function(f,e){
if(f)
{
c(false,null);
return;
}
doRawXmailRequest(a,d,e.xmailSid,function(i,h){
var g;
if(i&&h)
{
if(b.raw)
...
g=JSON.parse(h);
if(g&&g.head&&g.head.ret===-20002)
{
setXmailSidExpired();
b.retry=b.retry?b.retry+1:1;
if(b.retry<=2)
{
setTimeout(function(){
doXmailRequest(a,d,c,b);
},0);
return;
}
}
if(g&&g.head&&g.head.ret===0&&g.body)
{
c(true,g.body);
}
else{
...
function goXmailPage(b,a)
{
a=a||getTop().mainFrame;
checkXmailSid(function(c){
if(c)
{
a.location.href=b+"&sid="+getCookie('xm_sid')+"&r="+getXmailRandomString();
}
else{
getTop().location.href='https://mail.qq.com/cgi-bin/loginpage?s=session_timeout&from=&r='+Math.random();
}
});
}
var xmailReportConfigArray=[];
var reportTaskTimer;
function xmailReport(a)
{
xmailReportConfigArray.push(a);
reportTaskTimer&&clearTimeout(reportTaskTimer);
reportTaskTimer=setTimeout(function(){
xmailReportInternal();
},500);
}
function xmailReportInternal()
{
var a=[];
E(xmailReportConfigArray,function(b){
a.push({itemname:b.itemName,value:b.value||1,expand:b.expand||'',logid:b.logId});
});
if(a.length)
{
doXmailRequest('info/report',{func:1,items:a});
}
xmailReportConfigArray=[];
}
function checkXmailSid(a)
{
doXmailRequest('login/checksid',null,a);
}
function checkXmailSidPeriodically(b,c)
{
var a={value:0};
var d=setInterval(function(){
if(a.value===c)
{
debug('checkXmailSid count up to limit: '+a.value+', abort!');
clearInterval(d);
return;
}
a.value+=1;
...
{
reportDeviceInfoInternal('login',window.navigator.userAgent);
if(gbIsIE)
{
reportDeviceInfoInternal('login_ie');
reportDeviceInfoInternal('login_ie_'+gnIEVer);
}
else if(gbIsEdge)
{
reportDeviceInfoInternal('login_edge');
}
else if(gbIsChrome)
{
reportDeviceInfoInternal('login_chrome');
if(gbIsWin)
{
reportDeviceInfoInternal('login_chrome_windows');
if(gbIsWin7)
{
reportDeviceInfoInternal('login_chrome_windows_7');
}
else if(gbIsWin10)
{
reportDeviceInfoInternal('login_chrome_windows_10');
}
}
else if(gbIsMac)
{
reportDeviceInfoInternal('login_chrome_mac');
}
}
else if(gbIsFF)
{
reportDeviceInfoInternal('login_firefox');
if(gbIsWin)
{
reportDeviceInfoInternal('login_firefox_windows');
if(gbIsWin7)
{
reportDeviceInfoInternal('login_firefox_windows_7');
}
else if(gbIsWin10)
{
reportDeviceInfoInternal('login_firefox_windows_10');
}
}
else if(gbIsMac)
{
reportDeviceInfoInternal('login_firefox_mac');
}
}
else if(gbIsSafari)
{
reportDeviceInfoInternal('login_safari');
}
else if(gbIsQB)
{
reportDeviceInfoInternal('login_qqbrowser');
if(gbIsWin)
{
reportDeviceInfoInternal('login_qqbrowser_windows');
if(gbIsWin7)
{
reportDeviceInfoInternal('login_qqbrowser_windows_7');
}
else if(gbIsWin10)
{
reportDeviceInfoInternal('login_qqbrowser_windows_10');
}
}
}
if(gbIsWin)
{
reportDeviceInfoInternal('login_windows');
if(gbIsWin7)
{
reportDeviceInfoInternal('login_windows_7');
}
else if(gbIsWin10)
{
reportDeviceInfoInternal('login_windows_10');
}
}
else if(gbIsMac)
{
reportDeviceInfoInternal('login_mac');
}
}
function reportDeviceInfoInternal(a,b)
{
b=b?b.replace(/,/g,'#_#'):'';
xmailReport({itemName:a,expand:b,logId:20376});
}
...
var gsMsgNoSubject="\u8BF7\u586B\u5199\u90AE\u4EF6\u4E3B\u9898",gsMsgNoMail="\u672A\u9009\u4E2D\u4EFB\u4F55\u90AE\u4EF6",gsMsgSend="\u90AE\u4EF6\u6B63\u5728\u53D1\u9001\u4E2D... ",gsMsgSave="&nbsp;&nbsp;&nbsp;\u90AE\u4EF6\u6B63\u5728\u4FDD\u5B58\u5230\u8349\u7A3F\u7BB1...",gsMsgSaveOk="\u90AE\u4EF6\u6210\u529F\u4FDD\u5B58\u5230\u8349\u7A3F\u7BB1",gsMsgAutoSave="&nbsp;&nbsp;&nbsp;\u90AE\u4EF6\u6B63\u5728\u4FDD\u5B58\u5230\u8349\u7A3F\u7BB1...",gsMsgAutoSaveOk="\u90AE\u4EF6\u5DF2\u81EA\u52A8\u4FDD\u5B58\u5230\u8349\u7A3F\u7BB1",gsMsgSendErrorSaveOK="\u4FE1\u4EF6\u5DF2\u88AB\u4FDD\u5B58\u5230\u8349\u7A3F\u7BB1",gsMsgSaveErr="\u90AE\u4EF6\u672A\u80FD\u4FDD\u5B58\u5230\u8349\u7A3F\u7BB1",gsMsgNoSender="\u8BF7\u586B\u5199\u6536\u4EF6\u4EBA\u540E\u518D\u53D1\u9001",gsMsgNoCardSender="\u8BF7\u586B\u5199\u6536\u4EF6\u4EBA\u540E\u518D\u53D1\u9001",gsMsgNoCard="\u8BF7\u9009\u4E2D\u8D3A\u5361\u540E\u518D\u53D1\u9001",gsMsgSettingOk="\u8BBE\u7F6E\u4FDD\u5B58\u6210\u529F",gsMsgLinkErr="\u7F51\u7EDC\u5E94\u7B54\u5931\u8D25",gsMsgCheatAlert="\u7CFB\u7EDF\u4F1A\u5C06\u6B64\u90AE\u4EF6\u79FB\u5165\u5230\u201C\u5783\u573E\u90AE\u4EF6\u201D\u4E2D\uFF0C\u5E76\u628A\u90AE\u4EF6\u5185\u5BB9\u63D0\u4EA4\u7ED9\u90AE\u7BB1\u7BA1\u7406\u5458\u3002\n\n\u60A8\u786E\u5B9A\u8981\u4E3E\u62A5\u6B64\u90AE\u4EF6\u5417\uFF1F",gsMsgSendTimeErr="\u60A8\u8BBE\u7F6E\u7684\u53D1\u9001\u65F6\u95F4\u4E0D\u5B58\u5728",gsMsgMoveMailSameFldErr="\u4E0D\u80FD\u79FB\u52A8\u5230\u76F8\u540C\u7684\u76EE\u5F55",gsMsgNoteAutoSave="\u8BB0\u4E8B\u5185\u5BB9\u5DF2\u81EA\u52A8\u4FDD\u5B58";
...
{
return T(['<hr align="left" style="margin: 0 0 10px 0;border: 0;border-bottom:1px solid #E4E5E6;height:0;line-height:0;font-size:0;padding: 20px 0 0 0;width: 50px;"/>']);
}
...
var b=getTop();
if(b.goUserInfo._sStatus=='loading')
{
return;
}
b.goUserInfo._sStatus='loading';
var c=T(["/cgi-bin/getcomposedata?t=signature&fun=compose&sid=$sid$&qzonesign=$qzonesign$&r=$rand$"]).replace({sid:getSid(),qzonesign:"",rand:now()});
QMAjax.send(c,{method:"GET",timeout:10000,headers:{"If-Modified-Since":"0","Cache-Control":"no-cache, max-age=0"},onload:function(d,e){
b.goUserInfo._bHasLoad=true;
var h=trim2(e);
if(d&&h.indexOf("(")==0)
{
b.goUserInfo._sStatus='ready';
var f=(function(){
return (new Function("return "+h))();
})();
b.goUserInfo.set(f);
bindAccount();
var g=f.RealAllSignature;
if(false&&g)
{
b.E(g,function(i,j){
if(b.weworkSignature.is(i[1],i[0]))
{
b.weworkSignature.refreshData(function(l,k){
if(l||k.mail_business_card_status!=3)
{
delete g[j];
}
});
...
b.goUserInfo._sStatus='init';
}
securityTip();
b.goUserInfo._doCallback();
...
function afterAutoSave(b,c,d,a,e)
{
var m=0,v,x;
try{
var t=getTop().getMainWin();
function g()
{
if(disableAll)
{
...
m=4;
var s=b.split(" |"),r=[],n=t.QMAttach.getExistList();
for(var z=0,k=n.length;z<k;z++)
{
var u=S("Uploader"+n[z],t);
if(u&&!u.disabled&&u.value!="")
{
r.push(u);
}
}
m=5;
var h=r.length;
for(var z=0,k=s.length-1;z<k;z++)
{
var f=false;
for(var G=0;G<=z&&G<h;G++)
{
if(!r[G].disabled&&r[G].value.indexOf(s[z])!=-1)
{
r[G].disabled=true;
f=true;
try{
...
{
r[G].parentNode.childNodes[1].innerText=s[z];
}
}
catch(q)
{
...
if(!f)
{
var w=s[z]+" |",l=b.indexOf(w);
if(l!=-1)
{
b=b.substr(0,l)+b.substr(l+w.length,b.length-l-w.length);
}
}
}
m=6;
m=7;
if(b&&S("fattachlist",t))
{
S("fattachlist",t).value+=b;
}
m=8;
m=9;
if(e==='autosave'||e==='note_autosave')
{
getTop().getMainWin().QMComposePage.showInfoAutoSave(d,e);
}
else{
showInfo(formatDate(new Date(),"$hh$:$mm$")+" "+getTop().gsMsgSendErrorSaveOK);
}
m=10;
var p=getTop().QMDialog("composeExitAlert");
var o=p&&p.S("btn_exit_notsave");
if(o&&o.isShow())
{
t.enableAutoSave();
return fireMouseEvent(o,"click");
}
m=11;
if(!a)
{
...
{
v=q.message;
debug(["afterAutoSave:",q.message,"eid:",m]);
}
}
...
loadJsFileToTop(["$js_path$com/kits/qmeditor/qqmail/release/editor515efe.js"]);
loadingBox({model:"\u53D1\u4FE1",js:["$js_path$libcompose50e2f3.js","$js_path$qmaddrinput24e6ae.js"],oncheck:function(){
...
{
var b=getTop().S(a);
return !!b&&b.className.indexOf('fdlist_width')!==-1;
}
function onShowMoreFoldersClick()
{
var b=getTop().S('morefunctionfolders');
var a=b&&b.style.display==='none';
showFolders('morefunction');
if(a)
{
if(isFolded('folder_resume')&&reportResumeExpose)
{
reportResumeExpose();
}
if(isFolded('folder_xmbook')&&reportXmBookExpose)
{
reportXmBookExpose();
}
}
}
function updateOnlineDocUnreadNum(c)
{
var b=_oTop.S("folder_onlinedoc"),a="\u5728\u7EBF\u6587\u6863";
if(!b)
{
return;
}
...
{
var f="([a-z0-9.!#$%&'+/=?^_`{|}~-]{3,}@(?:[A-Z0-9-]+\\.)+[A-Z]{2,4})",g='[^\\s`!()\uFF08\uFF09\\[\\]{};\'"<>\u201C\u201D\u2018\u2019\u3002\uFF0C\uFF1B]',h=['(','(?:https?:\\/\\/|www\d{0,3}[.]|[a-z0-9.\\-]+[.][a-z]{2,4}\\/)','(?:','(?:&amp;amp;)','|','(?:&amp;)','|',g,'|','(?:[(\uFF08]',g,')','|','(?:[(\uFF08]',g,'*?[(\uFF08]',g,'*?[)\uFF09][)\uFF09])',')+','(?:','(?:[(\uFF08]',g,')','|','(?:[(\uFF08]',g,'*?[(\uFF08]',g,'*?[)\uFF09][)\uFF09])','|',g,')',')'].join(""),i=['(','[=\\u2248]?','(?:','(?:\uFF08\\+?\\d{1,5}\\-?\uFF09|\uFF08\\+?\\d{1,5}\\-?|\\(\\+?\\d{1,5}\\-?\\)|\\(\\+?\\d{1,5}\\-?|\\+?\\d{1,5}\\-?)?','(?:','1\\d{3}(?:\\-?\\d{3,4}){2}','|','(?:[(\uFF08]?\\d{2,4}[)\uFF09]?)?\\d{3,5}\\-?\\d{3,5}(?:\\-?(?:\\d{1,8}|\uFF08\\d{1,8}\uFF09|\\(\\d{1,8}\\)))?',')',')','(?:(?![\\d\\-\\+\\*\\/=\\u2248]))',')'].join(""),e=(/((?!0000)[0-9]{4}(?:[-/.]{1})?(?:(?:0?[1-9]|1[0-2])(?:[-/.]{1})?(?:0?[1-9]|1[0-9]|2[0-8])|(?:0?[13-9]|1[0-2])(?:[-/.]{1})?(?:29|30)|(?:0?[13578]|1[02])(?:[-/.]{1})?31)|(?!0000)[0-9]{4}(?:[-/\u5E74]{1})(?:(?:0?[1-9]|1[0-2])\u6708(?:0?[1-9]|1[0-9]|2[0-8])\u65E5|(?:0?[13-9]|1[0-2])\u6708(?:29|30)\u65E5|(?:0?[13578]|1[02])\u670831\u65E5)|(?!\u5E74)(?:(?:0?[1-9]|1[0-2])\u6708(?:0?[1-9]|1[0-9]|2[0-8])\u65E5|(?:0?[13-9]|1[0-2])\u6708(?:29|30)\u65E5|(?:0?[13578]|1[02])\u670831\u65E5)|(?![/-])(?:(?:0?[1-9]|1[0-2])(?:[-/.]{1})?(?:0?[1-9]|1[0-9]|2[0-8])|(?:0[13-9]|1[0-2])(?:[-/.]{1})?(?:29|30)|(?:0[13578]|1[02])(?:[-/.]{1})?31))\b/g).source;
function a(j)
{
...
return c.replace(new RegExp([h,f,e,i].join("|"),"ig"),function(m,l,k,j,n){
var r;
if(l)
{
var p=m.indexOf("http")!=0?"http://"+m:m;
r=['<a href="',p,'">',a(m),'</a>'].join("");
}
else if(k)
{
var p=m;
var q=p.match(/(.*)@/);
q&&(q=q[1]);
if(q&&q.length>=3&&(!q.match(/^\d+$/)||q.match(/^\d+$/)&&+q>=10000))
{
r=['<a href="','mailto:',p,'">',a(m),'</a>'].join("");
}
else{
r=m;
}
...
if(b)
{
var o=b.parentNode.firstChild,s=o.innerText||o.textContent||o.nodeValue||"";
if(s.charAt(s.length-1)!=":"&&/((?!0000)[0-9]{4}([-/]{1})(?:(?:0?[1-9]|1[0-2])(?:[-/]{1})(?:0?[1-9]|1[0-9]|2[0-8])|(?:0?[13-9]|1[0-2])(?:[-/]{1})(?:29|30)|(?:0?[13578]|1[02])(?:[-/]{1})31)|(?!0000)[0-9]{4}(?:[-/\u5E74]{1})(?:(?:0?[1-9]|1[0-2])\u6708(?:0?[1-9]|1[0-9]|2[0-8])\u65E5|(?:0?[13-9]|1[0-2])\u6708(?:29|30)\u65E5|(?:0?[13578]|1[02])\u670831\u65E5)|(?!\u5E74)(?:(?:0?[1-9]|1[0-2])\u6708(?:0?[1-9]|1[0-9]|2[0-8])\u65E5|(?:0?[13-9]|1[0-2])\u6708(?:29|30)\u65E5|(?:0?[13578]|1[02])\u670831\u65E5)|(?![/-])(?:(?:0?[1-9]|1[0-2])(?:[-/]{1})(?:0?[1-9]|1[0-9]|2[0-8])|(?:0[13-9]|1[0-2])(?:[-/]{1})(?:29|30)|(?:0[13578]|1[02])(?:[-/]{1})31))/g.test(m))
{
var t=c.substring(arguments[arguments.length-2]+m.length,arguments[arguments.length-2]+m.length+6);
!t.match(/\s?\d\d:\d\d/g)&&(t="");
r="<span style='border-bottom:1px dashed #ccc;' t='5' times='"+t+"' >"+m+"</span>";
}
else{
r=m;
}
...
if(d.indexOf('https://wx.mail.qq.com')>-1)
{
return false;
}
...
{
var v=a.href.split("mailto:")[1];
if(v=="mail_qq@qq.com"&&attr(a.parentNode,"subject"))
{
v+=("&initsubject="+attr(a.parentNode,"subject"));
}
window.open(["/cgi-bin/readtemplate?sid=",getSid(),"&t=compose&s=cliwrite&newwin=true&email=",v].join(""));
return false;
}
else if(/qqmail_card_reply$/i.test(a.className)||/qqmail_card_reply_btn$/i.test(a.className)||/qqmail_birthcard_reply$/i.test(a.className)||/qqmail_birthcard_reply_btn$/i.test(a.className)||/(http:|https:)?(\/\/)i.mail.qq.com\/cgi-bin\/uma_compose_card/i.test(a.href))
{
var u=a.name,s=a.className,f=!!u,d=s.indexOf("birthcard")!=-1;
getMainWin().location=T('/cgi-bin/cardlist?sid=$sid$&t=$t$&s=$s$&today_tips=$tips$&loc=readmail,readmail,sendnewcard,1&ListType=$listtype$&email=$email$$newwin$').replace({sid:getSid(),t:f?"compose_card":"card",s:d?"replybirthcard":"",tips:s.indexOf("btn")!=-1?"112":"111",listtype:f?"No":"Cards&Cate1Idx=listall",email:u,newwin:getTop().bnewwin?"&newwin=true":""});
return false;
}
...
var K=a.href;
K=trim2(K);
QMAjax.send(K,{method:"POST",content:"sid="+getSid(),onload:function(e,R){
if(e)
{
if(/<!--cgi exception-->/ig.test(R))
{
var W=(/<!--cgierrorcode:-([\d]+)-->/ig.test(R)&&RegExp.$1);
if("821"==W)
{
showError("\u5BF9\u65B9\u5DF2\u505C\u6B62\u5171\u4EAB\u65E5\u5386");
}
...
{
var V=(/<!--email:([\W\w]+?)-->/ig.test(R)&&RegExp.$1);
showError("\u8BF7\u767B\u5F55\u8D26\u53F7"+V+"\u91CD\u65B0\u52A0\u5165\u65E5\u5386");
}
else if("823"==W)
{
showError("\u9080\u8BF7\u90AE\u4EF6\u5DF2\u8FC7\u671F");
...
{
var U=getTop();
U.goUrl(U,[U.location.protocol,"//",U.location.hostname,"/cgi-bin/loginpage"].join(""));
}
else{
showError("\u52A0\u5165\u5931\u8D25");
}
...
else if(/qqmail_appleid_verify_password_btn/i.test(a.className))
{
getTop().toVerifyQQAccount();
return false;
}
else if(/qqmail_appleid_verify_code_changeBtn/i.test(a.className))
{
var i=parents("td.qqmail_appleid_verify_code_container",a),j=finds("td.qqmail_appleid_verify_code_desc",i[0])[0],o=finds("a.qqmail_appleid_verify_code_sendBtn",i[0])[0],I=attr(o,"data-type"),q={"appleid":"appleid_wx","appleid_wx":"appleid"},k={"appleid":"\u9A8C\u8BC1\u7801\u53D1\u9001\u81F3 \u624B\u673AQQ-\u670D\u52A1\u53F7-QQ\u5B89\u5168\u4E2D\u5FC3","appleid_wx":"\u9A8C\u8BC1\u7801\u53D1\u9001\u81F3 \u624B\u673A\u5FAE\u4FE1-\u670D\u52A1\u53F7-\u5FAE\u4FE1\u5B89\u5168\u4E2D\u5FC3"},h={"appleid":"\u5207\u6362\u81F3QQ\u5B89\u5168\u9A8C\u8BC1","appleid_wx":"\u5207\u6362\u81F3\u5FAE\u4FE1\u5B89\u5168\u9A8C\u8BC1"},D=q[I];
if(D)
{
j.innerHTML=k[D];
attr(o,"data-type",D);
...
{
var z=attr(a,"data-mailid");
var I=attr(a,"data-type");
if(I!=="appleid"&&I!=="appleid_wx")
{
return;
}
loadCssFile("$css_path$login/addAccount493ddf.css",true,p.document);
function P()
{
showInfo("\u53D1\u9001\u9A8C\u8BC1\u7801...");
QMAjax.send("/cgi-bin/gen_safecode",{method:"POST",content:["t=apple_id_verify.json&mailid=",encodeURIComponent(z),"&type=",encodeURIComponent(I)].join(""),onload:function(e,R){
var U=evalValue(R||"{}");
if(e&&U&&U.resCode=="0")
{
showInfo("\u9A8C\u8BC1\u7801\u5DF2\u53D1\u9001");
...
function L()
{
var R=g.S("ipt_username"),e=R.value||"";
if(e.length<6)
{
showError("\u8BF7\u8F93\u51656\u4F4D\u9A8C\u8BC1\u7801");
return;
}
QMAjax.send("/cgi-bin/gen_safecode",{method:"POST",content:["type=appleid_check&t=apple_id_verify.json&mailid=",encodeURIComponent(z),"&safecode=",encodeURIComponent(e)].join(""),onload:function(U,V){
var W=evalValue(V||"{}");
if(U&&W&&W.resCode=="0")
{
showInfo("\u9A8C\u8BC1\u901A\u8FC7\uFF0C\u8BFB\u4FE1\u4E2D...");
...
if(/<!--cgi exception-->/ig.test(V))
{
var Y=(/<!--cgierrorcode:-([\d]+)-->/ig.test(V)&&RegExp.$1);
if("2"==Y)
{
goUrl(window,[location.protocol,"//",location.hostname,"/cgi-bin/loginpage"].join(""));
}
...
var R=this,e=R.S("ipt_username");
e.focus();
R.S("geConfirmBtn").onclick=function(){
L();
};
R.S("geCancelBtn").onclick=function(){
R.close();
};
e.onkeydown=function(U){
var V=U||window.event;
if(V.keyCode===13)
{
L();
preventDefault(V);
...
e.oninput=e.onpropertychange=function(U){
...
{
var u=attr(a,"email"),O=["\u5929\u4E86\u565C\uFF0C\u592A\u611F\u4EBA\u4E86\uFF0C\u8C22\u8C22\u4F60\u7684\u8D3A\u5361\uFF01","\u6536\u5230\u4F60\u7684\u8D3A\u5361\uFF0C\u771F\u662F\u592A\u5F00\u5FC3\u5566\uFF01\uFF01","\u597D\u5F00\u5FC3\u54E6\uFF0C\u8C22\u8C22\u4F60\u7684\u8D3A\u5361\u54E6\uFF01","\u8C22\u8C22\u4F60\u7684\u8D3A\u5361\uFF0C\u5E38\u8054\u7CFB\uFF01","\u6709\u4EBA\u60E6\u8BB0\u7684\u611F\u89C9\u771F\u597D\uFF0C\u611F\u8C22\u6709\u4F60\uFF01","\u8C22\u8C22\u4F60\u5728\u8FD9\u4E2A\u7F8E\u597D\u7684\u65E5\u5B50\u8BB0\u5F97\u6211\uFF01\u5E38\u8054\u7CFB\uFF01","\u6536\u5230\u4F60\u7684\u8D3A\u5361\u5566\uFF0C\u611F\u52A8\u5F97\u4E0D\u8981\u4E0D\u8981\u7684\u3002\u3002\u3002","\u563F\uFF0C\u6211\u6536\u5230\u8D3A\u5361\u4E86\u54E6\uFF0C\u597D\u68D2\u5594\uFF0C\u8C22\u8C22\u4F60\uFF01","\u8C22\u8C22\u4F60\u7684\u8D3A\u5361\uFF0C\u6211\u7684\u670B\u53CB\uFF01","\u8C22\u8C22\u4F60\u7684\u8D3A\u5361\uFF0C\u4E5F\u795D\u4F60\u5E78\u798F\u5FEB\u4E50\uFF01"],N=O[(Math.floor(Math.random()*O.length))];
openComposeDlg("card",{sTitle:"\u7B54\u8C22\u597D\u53CB",sDefAddrs:u,bAddrEdit:true,nWidth:495,sDefContent:N,bContentEdit:true,sDefSubject:"\u8C22\u8C22\u4F60\u7684\u8D3A\u5361!",bRichEditor:false,oncomplete:function(){
},bShowResult:true});
return false;
}
else if(/qqmail_card_reply_thanksbtn$/i.test(a.className)||/qqmail_card_reply_thanks$/i.test(a.className)||/qqmail_birthcard_reply_thanksbtn$/i.test(a.className))
{
var u=a.name;
openComposeDlg("card",{sTitle:"\u7B54\u8C22\u597D\u53CB",sDefAddrs:u,bAddrEdit:true,nWidth:495,sDefContent:"\u8C22\u8C22\u4F60\u7684\u8D3A\u5361\uFF01 \u4EE5\u540E\u8981\u5E38\u8054\u7CFB\u54E6\u3002",bContentEdit:true,sDefSubject:"\u8C22\u8C22\u4F60\u7684\u8D3A\u5361!",bRichEditor:false,oncomplete:function(){
},bShowResult:true});
return false;
}
...
{
var u=a.name;
openComposeDlg("postcard",{sTitle:"\u7B54\u8C22\u597D\u53CB",sDefAddrs:u,bAddrEdit:true,nWidth:495,sDefContent:"\u8C22\u8C22\u4F60\u7684\u660E\u4FE1\u7247\uFF01 \u4EE5\u540E\u8981\u5E38\u8054\u7CFB\u54E6\u3002",bContentEdit:true,sDefSubject:"\u8C22\u8C22\u4F60\u7684\u660E\u4FE1\u7247!",bRichEditor:false,oncomplete:function(){
},bShowResult:true});
return false;
}
...
goUrlMainFrm(T('/cgi-bin/readtemplate?sid=$sid$&t=compose_postcard&email=$email$&reply=1&frname=$name$&fraddr=$addr$').replace({name:escape(y),addr:escape(x),sid:getSid(),email:a.name}),false);
return false;
}
...
else if(a.className=="weworkSignature_link"&&a.href&&/^https?:\/\/work\.weixin\.qq\.com\/wework_admin\/user\/h5\/qqmail_user_card\//.test(a.href))
{
var p=getTop();
var G=a.href;
G+=G.indexOf('?')==-1?'?':'&';
p.weworkSignature.refreshData(function(R,e){
var U=[];
U.push('sid='+p.getSid());
U.push('uin='+p.getUin());
U.push('email='+encodeURIComponent(p.getUserInfo("addr")||p.getCookie("qqmail_alias")||''));
if(e&&e.mail_business_card_status==3)
{
U.push('has_received=true');
}
G+=U.join('&');
window.open(G);
});
...
{
var t=a.getAttribute("ctype");
var C=a.getAttribute("param_new");
var K="";
if(C.indexOf("follow=1")>=0)
{
var w=a.getAttribute("followuin");
K=(getTop().gsRssDomain||"")+"/cgi-bin/reader_mgr";
QMAjax.send(K,{method:"POST",content:"fun=followshare&followuin="+w+"&sid="+getSid(),onload:function(e,R){
if(e)
{
getMainWin().location=T('$host$/cgi-bin/reader_article_list?sid=$sid$&$param$').replace({host:(getTop().gsRssDomain||""),sid:getSid(),param:C});
}
}});
}
else{
...
if(t=="onefeed")
{
K=(getTop().gsRssDomain||"")+"/cgi-bin/reader_mgr?fun=setlog&flag=3&from=2";
}
else{
K=(getTop().gsRssDomain||"")+"/cgi-bin/reader_mgr?fun=setlog&flag=3&from=4";
}
runUrlWithSid(K);
return false;
}
...
var K=(getTop().gsRssDomain||"")+"/cgi-bin/reader_mgr?fun=setlog&flag=3&from=3";
runUrlWithSid(K);
return false;
}
...
var B=getMainWin().location.href;
if(b!="preview"&&B.indexOf('/cgi-bin/readmail?')==-1&&B.indexOf('t=mail_list_ad')==-1&&B.indexOf('t=unreadmail')==-1&&B.indexOf('t=r_index')==-1)
{
return true;
}
c||(c=getTop().getUrlParams(getMainWin().location)['mailid']);
var J=''+getTop().g_uin;
var H=c&&encryptStr(J.substr(J.length-3)+c);
H||(H=c);
var Q=window.open(T('$baseDomain$/cgi-bin/mail_spam?action=check_link&url=$url$&mailid=$mid$&spam=$spam$').replace({baseDomain:getTop().getTopHost(),mid:H,spam:b=="spam"?1:0,url:encodeURI(a.href)}),"_blank");
Q&&(Q.opener=null);
return false;
}
...
function adjustLocalTime(e,c,g)
{
function f(q)
{
var l=new Date(q*1000);
var k=l.getFullYear();
var j=l.getMonth()+1;
var i=l.getDate();
var n=l.getHours();
var p=l.getMinutes();
var r='\uFF08\u661F\u671F'+"\u65E5\u4E00\u4E8C\u4E09\u56DB\u4E94\u516D".charAt(l.getDay())+'\uFF09';
var o=new Date(q).toDateString()===new Date().toDateString();
return (o?'\u4ECA\u5929':k+'\u5E74'+j+'\u6708'+i+'\u65E5'+r)+(n<12?'\u4E0A\u5348':'\u4E0B\u5348')+(n<=12?n:n-12)+' : '+(p<10?'0'+p:p);
}
function d()
{
var l=new Date().getTimezoneOffset();
var k=(Math.floor(-l/60)*100)+'';
if(!c)
{
c='800';
}
...
function checkSid()
{
QMAjax.send('/cgi-bin/checksid?r='+Math.random(),{onload:function(b,a){
debug('checksid onload, success: '+b+', response: '+a+', date: '+new Date().toLocaleTimeString());
}});
}
var checkSidTime=0;
function resetCheckSidTime(b,a)
{
debug('resetCheckSidTime, times: '+checkSidTime+', from: '+a+', url: '+b+', date: '+new Date());
checkSidTime=0;
}
function checkSidPeriodically()
{
var a=setInterval(function(){
debug('checkSidPeriodically, times: '+checkSidTime+new Date());
if(checkSidTime===50)
{
debug('checksid count up to limit: '+checkSidTime+', abort!');
return;
}
checkSidTime+=1;
checkSid();
...
{
cacheByIframe(a&&a.defcss==false?[]:[["css","",getRes("$css_path$comm493ddf.css")],["css",getPath("style"),"skin"]],extend({className:"menu_base_if",transparent:false,destroy:false},a,{win:b,header:["<script>",getTop.toString().replace(/[\r\n]/g,""),"<\/script>",a&&a.header||""].join(""),onload:function(c){
...
loadJsFile("$js_path$qmlinkman4f37d5.js",true,e,b);
...
if(!b.QMPlayer)
{
loadJsFileToTop(["$js_path$qmplayer/player38b933.js"]);
}
waitFor(function(){
return !!b.QMPlayer;
},function(c){
...
{
var b=getTop();
b.loadCssFile("$css_path$webpushtip493ddf.css",true);
...
var weworkSignature=(function(){
var e=getTop();
var b;
function p(x)
{
var z=x=='compose'?e.T('//work.weixin.qq.com/wework_admin/qqmail_card/login_page?sid=$sid$&uin=$uin$&email=$email$&from=$from$'):e.T('//work.weixin.qq.com/wework_admin/qqmail_card/intro_receive?sid=$sid$&uin=$uin$&email=$email$&from=$from$&qqmail=true');
var B=z.replace({sid:e.getSid(),uin:e.getUin(),email:encodeURIComponent(e.getUserInfo("addr")||e.getCookie("qqmail_alias")||''),from:encodeURIComponent(x)});
if(x=='compose')
{
e.weworkSignature.ossLog('qqmail_card_bubble_clickget');
}
if(e.gbIsIE)
{
e.confirmBox({title:"QQ\u90AE\u7BB1\u4F01\u4E1A\u540D\u7247",mode:"prompt",msg:'\u662F\u5426\u5B8C\u6210QQ\u90AE\u7BB1\u4F01\u4E1A\u540D\u7247\u7684\u7ED1\u5B9A\uFF1F',confirmBtnTxt:'\u5DF2\u7ED1\u5B9A',cancelBtnTxt:'\u53D6\u6D88',onreturn:function(C){
switch(x)
{case 'compose':
u();
break;
...
var y=function(D){
var G=D.origin||(D.originalEvent&&D.originalEvent.origin);
var C=D.data;
if(!C||(G&&G.substr(-18)!='work.weixin.qq.com')||C.sid!=e.getSid()||C.uin!=e.getUin())
{
return;
}
if(C.action=='showmsg')
...
b=new (e.QMDialog)({sId:'weworkSignature_dlg',sTitle:'QQ\u90AE\u7BB1\u4F01\u4E1A\u540D\u7247',sUrl:B,nWidth:340,nHeight:470,bFrameScrolling:true,onbeforeclose:function(){
b=null;
if(y&&window.removeEventListener)
{
...
{
e.addEventListener('message',y,false);
}
}
function i()
{
...
return TE(['<div style="display: table"><a target="_blank" contenteditable="false" href="$link$" class="weworkSignature_link" style="text-decoration: none;">','<table data-wwcard="true" cellspacing="0" cellpadding="0" style="margin: 12px; margin-left:0;background-color:#fff;border-radius:4px;overflow:hidden;border: 1px solid #E6E6E6">','<tbody>','<tr>','<td colspan="3" height="10" style="line-height: 1;"><div style="width: 10px; height: 10px;"></div></td>','</tr>','<tr>','<td width="12"><div style="width: 12px; height: 12px;"></div></td>','<td valign="top">','<table cellspacing="0" cellpadding="0">','<tbody>','<tr>','<td valign="top" align="left">','<div style="font-size: 16px; font-weight: bold; color: #000; line-height: 1.3;min-width: 60px;max-width:250px; min-width: 130px; word-break: break-word; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">$name$</div>','<div style="margin-top: 12px">','$@$if($job$)$@$','<div style="font-size: 12px; color: #353B42; margin-top: 6px; line-height: 1.3;max-width:250px;word-break: break-word; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">$job$</div>','$@$endif$@$','$@$if($corp_name$)$@$','<div style="font-size: 12px; color: #353B42; margin-top: 6px; line-height: 1.3;max-width:250px;word-break: break-word; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">$corp_name$</div>','$@$endif$@$','</div>','</td>','<td width="20"><div style="width: 20px; height: 12px;"></div></td>','<td valign="top" align="right" style="text-align: right; font-size: 12px; color: #1F6CD7; line-height: 16px;">','<div style="width: 75px;">','<a target="_blank" href="$link$" class="weworkSignature_link" style="display: inline-block; background: #f2f5f7; border-radius: 12px; padding: 3px 11px 2px; color: #1F6CD7;line-height: 1.5; text-decoration: none;">','<img src="https://rescdn.qqmail.com/qqmail/images/wework_signature_qrcode_2019061715.png" srcset="https://rescdn.qqmail.com/qqmail/images/wework_signature_qrcode_2019061715_2x.png 2x, https://rescdn.qqmail.com/qqmail/images/wework_signature_qrcode_2019061715_3x.png 3x" width="11" height="11" style="width: 11px; height: 11px; vertical-align: -1px;"> \u8054\u7CFB\u6211','</a>','</div>','<div style="height: 1px; overflow:hidden;"><img src="https://mail.qq.com/cgi-bin/get_buss_card?func=buss_kv_log&kvstring=qqmail_card_rece&img=true" style="width: 1px; height: 1px;"></div>','</td>','</tr>','</tbody>','</table>','</td>','<td width="12"><div style="width: 12px; height: 12px;"></div></td>','</tr>','<tr>','<td colspan="3" height="10" style="line-height: 1;"><div style="width: 10px; height: 10px;"></div></td>','</tr>','</tbody>','</table>','</a></div>']).replace({name:e.htmlEncode(x.name),job:e.htmlEncode(x.job),corp_name:e.htmlEncode(x.corp_name),link:e.htmlEncode(x.qrcode_url)});
}
function l(y,x)
{
return (y=='\u5546\u52A1\u6807\u8BC6'||y=='\u5546\u52A1\u7B7E\u540D')&&x&&(x=='wwcard'||x.indexOf(' data-wwcard="true"')!=-1);
}
var a=[];
var g='init';
var d=null;
...
{
setTimeout(function(){
e.callBack.call(y,x,[null,d]);
});
return;
}
g='wait';
e.QMAjax.send("/cgi-bin/get_buss_card?resp_charset=UTF8&sid="+e.getSid(),{method:"GET",onload:function(z,B){
g='init';
var C;
if(!z)
{
C=[new Error('http_err')];
}
else{
var D=e.evalValue(B);
g='ready';
...
e.callBack.call(y,x,C);
e.E(a,function(G){
try{
e.callBack.call(y,G,C);
}
catch(H)
{
}
...
B.push('addr='+encodeURIComponent(D));
}
});
if(B.length)
{
e.QMAjax.send("/cgi-bin/get_buss_card?resp_charset=UTF8&sid="+e.getSid()+"&func=getfriendbusscard&mailid="+z+"&"+B.join('&'),{method:"GET",onload:function(D,G){
var H=e.evalValue(G);
if(H&&H.list&&H.list.length)
{
e.E(H.list,function(I){
c[I.addr]=C[I.addr]=I;
});
}
e.callBack.call(this,x,[null,C]);
}});
}
else{
e.callBack.call(this,x,[null,C]);
...
function v(x)
{
e.goUserInfo.reset(x);
g='init';
c={};
}
function u()
{
var y=e.goUserInfo.get('RealAllSignature');
var x=0;
e.E(y,function(){
x++;
});
...
var C=e.getMainWin();
var D=C.location;
var G=C.QMComposePage;
if(!G||D.href.indexOf('t=compose')==-1)
{
return;
}
var B=e.goUserInfo.get('RealAllSignature');
var z=0;
e.E(B,function(){
z++;
});
G.setMultiSignatureSelect();
if(x!=z)
{
G.setSignature('sign',e.goUserInfo.get('RealUserSignatureId'));
}
return;
n(function(I,H){
if(I||H.mail_business_card_status!=3)
{
return;
}
G.setMultiSignatureSelect();
...
var z=[];
var B={};
y.E(y.finds('span[mailid]',x),function(C){
var D=y.attr(C,'e');
if(D&&!B[D])
{
B[D]=true;
z.push('addr='+encodeURIComponent(D));
}
});
n(function(D,C){
if(C.mail_business_card_status==3)
{
y.addClass(x.document.body,'ww_signature_tips_btn_NoMoreInfo');
}
});
y.QMAjax.send("/cgi-bin/get_buss_card?resp_charset=UTF8&func=addr_batch&sid="+y.getSid(),{method:'POST',content:z.join('&'),onload:function(C,D){
if(!C)
{
return;
}
var H=y.evalValue(D);
var G={};
y.E(H.list,function(J){
if(!J)
{
return;
}
G[J.addr]=J.mail_business_card_status;
});
var I=false;
y.E(y.finds('span[mailid]',x),function(J){
var K=y.attr(J,'e');
if(!K)
{
return;
}
var L=y.attr(J,'mailid');
if(!L)
{
return;
}
...
if(G[K]==3)
{
y.attr(J,'r','wwprofile');
}
y.insertHTML(J,'afterEnd','<span ignore_profile_tips="true" class="ww_signature_tips_btn ww_signature_tips_btn_MailList"><input type="button" class="ww_signature_icon ww_signature_icon_MailIcon" /><div class="ww_signature_tips">\u5546\u52A1\u7528\u6237\u6807\u8BC6<span ignore_profile_tips="true" class="ww_signature_tips_linkWrap">\u3002<a class="ww_signature_tips_link" href="javascript:;" onclick="getTop().weworkSignature.showDialogInEl(this,\'mail_list\')" nord="true">\u4E86\u89E3\u66F4\u591A</a></span></div></span>');
y.addClass(J,'accountNameWithIcon');
y.addClass(J.parentNode.parentNode,'forAccountNameWithIcon');
y.rmClass(J.parentNode.parentNode,'tf');
I=true;
}
});
if(I)
{
y.weworkSignature.ossLog('qqmail_cardicon_rece');
}
}});
}
function q(x,y)
...
switch(y)
{case 'today:main':
e.QMAjax.send("/cgi-bin/do",{content:"action=setbitmap&key=wework_signature_today_main&value=1",method:"POST",onload:function(C){
}});
break;
case 'compose':
var z=e.S('weworkSignatureTips',B);
if(z)
{
e.removeSelf(z);
}
...
function s()
{
e.confirmBox({title:"\u5173\u95ED\u6211\u7684\u5546\u52A1\u7B7E\u540D",mode:"prompt",msg:'\u5173\u95ED\u540E\uFF0C\u5546\u52A1\u7B7E\u540D\u5C06\u5931\u6548\uFF0C\u662F\u5426\u5173\u95ED\u5546\u52A1\u7B7E\u540D\uFF1F',onreturn:function(x){
if(x)
{
e.QMAjax.send("/cgi-bin/get_buss_card?resp_charset=UTF8&func=unbind_card&sid="+e.getSid(),{method:'POST',onload:function(y,z){
if(y)
{
showInfo('\u5173\u95ED\u6210\u529F');
}
...
j();
w();
e.weworkSignature.ossLog('qqmail_card_info_clickclose');
}});
}
}});
}
...
function r(x)
{
f=new (e.QMDialog)({sId:'weworkSignatureUnbind_dlg',sTitle:'QQ\u90AE\u7BB1\u4F01\u4E1A\u540D\u7247',sUrl:e.T('/cgi-bin/get_buss_card?sid=$sid$&t=get_buss_card_unbind&from=$from$').replace({sid:e.getSid(),from:encodeURIComponent(x)}),nWidth:340,nHeight:470,bFrameScrolling:true,onbeforeclose:function(){
f=null;
}});
if(x=='mail_list'||x=='readmail')
{
e.weworkSignature.ossLog('qqmail_cardicon_rece_readmore');
}
else if(x=='today:right')
{
e.weworkSignature.ossLog('qqmail_card_info_click');
}
}
function j()
{
...
function m(x)
{
e.QMAjax.send("/cgi-bin/get_buss_card?resp_charset=UTF8&func=buss_kv_log&uin="+e.getUin()+'&kvstring='+encodeURIComponent(x),{method:'GET'});
}
return {is:l,unbind:s,render:o,ossLog:m,showDialog:p,closeDialog:i,showUnbindDialog:r,closeUnbindDialog:j,refreshData:n,updateTodayPage:w,updateComposePage:u,showDialogInEl:q,appendWeworkCardIcon4mailList:h,getProfileInfo:k};
})();
function all_js()
{
}
...
h.sUrl=i.TE("/cgi-bin/ftnviewcompress?sid=$sid$&cpsfile=$@$eval getTop().encodeURI($sName$)$@$&fid=$sFileId$&action=list&t=cps.json&fromattach=1&code=$sFetchCode$&k=$sKey$").replace(f);
}
}
else if(k!="doc")
{
...
else if(m=="pdf"&&i.gbIsChrome)
{
h.sUrl=i.TE(["/cgi-bin/xmail?xmmodule=ftn&xmcgi=preview&key=$k$&code=$code$&sid=$sid$"]).replace(g);
}
else{
...
function onResumeFolderClick(a)
{
goResumePage();
reportResumeClick();
if(a)
{
disableResumeRedPoint();
}
}
function goResumePage()
{
goXmailPage('https://wx.mail.qq.com/home/readtemplate?name=resume_homepage.html');
}
function disableResumeRedPoint()
{
var a=S('resume_red_new',window);
if(a)
{
a.style.display='none';
}
}
function reportResumeFold()
{
reportResume('Resume_web_sidebar_entrance_fold');
}
function reportResumeExpose()
{
reportResume('Resume_web_sidebar_entrance_expose');
}
function reportResumeClick()
{
reportResume('Resume_web_sidebar_entrance_click');
}
function reportResume(a)
{
xmailReport({itemName:a,expand:'2,,,',logId:19455});
}
function jumpToResume()
{
getTop().mainFrame.location.href='https://wx.mail.qq.com/resume/login?enc=3&sid='+getSid()+'&cookie_sid='+encodeURIComponent(getCookie('sid'));
openResume(function(){
setTimeout(function(){
getTop().switchFolder('folder_resume');
disableResumeRedPoint();
},500);
});
}
function openResume(a)
{
QMAjax.send(T('/cgi-bin/setting4?t=setting12&sid=$sid$&submit_from=resume').replace({sid:getSid()}),{content:'Fun=submit&openresume=1&f=json',method:"POST",onload:function(b,c){
if(b)
{
...
function onXmBookFolderClick()
{
goXmBookPage();
reportXmBookClick();
disableXmBookRedPoint();
}
function goXmBookPage()
{
goXmailPage('https://wx.mail.qq.com/home/readtemplate?name=readsubscribe.html');
}
function disableXmBookRedPoint()
{
var a=S('xmbook_red_point',window);
if(a)
{
a.style.display='none';
}
}
function reportXmBookFold()
{
reportXmBook('Read_web_application_fold');
}
function reportXmBookExpose()
{
reportXmBook('Read_web_application_expose_btn');
}
function reportXmBookClick()
{
reportXmBook('Read_web_folder_click');
}
function reportXmBook(a)
{
xmailReport({itemName:a,expand:',,,,,',logId:16292});
}
...
{
var b=getTop(),a=arguments.callee;
b.loadCssFile("$css_path$login/addAccount493ddf.css",true,b.document);
...
if(!this._msVer)
{
var a=getTop().getRes("$css_path$addAccountPtlogin412d4f.css"),b=(/addAccountPtlogin(.*)\.css/.test(a)&&RegExp.$1)||"";
this._msVer=b;
}
return this._msVer;
...
else if(_sEmail&&_sEmail.indexOf("@gmail")>-1)
{
c.showGmailAlert(a);
}
else{
...
},showGmailAlert:function(a){
var c=this,b=c._oDialog;
b.setHtml("_body_",_oTop.T(['<div style="padding: 20px 60px 40px;">','<div style="font-size: 14px;font-weight: bold;padding: 0px 0 10px 0;">\u8BF7\u524D\u5F80\u79FB\u52A8\u7AEFQQ\u90AE\u7BB1\u8FDB\u884C\u6DFB\u52A0</div>','<div style="padding-bottom: 20px;">','<div style="color: #999896;line-height: 17px;">\u7531\u4E8E\u6280\u672F\u95EE\u9898\uFF0CQQ\u90AE\u7BB1\u7F51\u9875\u7248\u6682\u4E0D\u652F\u6301Gmail\u8D26\u6237\u6DFB\u52A0\u3002</div>','<div style="position:relative">','<a style="line-height: 17px;text-decoration: underline;color: blue;" class="qm_qrCodeTipsContainerInAddGmailTutorial">\u626B\u7801\u4E0B\u8F7D\u79FB\u52A8\u7AEFQQ\u90AE\u7BB1','<div class="qm_qrCodeTipsInAddGmailTutorial">','<div class="qm_qrCodeinAddGmailTutorial"></div>','</div>','</a>','</div>','</div>','<div class="qm_addGmailTutorial"></div>','</div>']));
...
if(!this._msVer)
{
var a=getTop().getRes("$css_path$qui_dialogPtlogin493ddf.css"),b=(/qui_dialogPtlogin(.*)\.css/.test(a)&&RegExp.$1)||"";
this._msVer=b;
}
return this._msVer;
},getDefaultCfg:function(){
...
function toVerifyQQAccount(d,e)
{
var c=getTop(),a=arguments.callee;
if(!d)
{
var b=c.S("useraddr");
d=getTop().goUserInfo.get("DEF_MAIL_FROM")||(b&&b.innerHTML)||(c.getUin()+'@qq.com')||'';
}
c.loadCssFile("$css_path$login/addAccount493ddf.css",true,c.document);
a._oDialog=new c.QMDialog({sId:"id_VerifyAccount",sTitle:"\u8EAB\u4EFD\u9A8C\u8BC1",nWidth:330,sClassName:"verifyAccount_dialog",sBodyHtml:a._oTmpl.replace({"username":d,"ptloginurl":a.getPtloginUrl({"uin":c.getUin()})}),bAnimation:false,onshow:function(){
var f=this;
getTop().ptlogin2_onResize=a.ptlogin2OnResize(f);
getTop().ptloginResize();
},onclose:function(){
if(this.reloadLeftWin)
{
...
var f=this;
f.S("importFrame")&&removeSelf(f.S("importFrame"));
}});
a.doAction=function(g,f){
if(f!=='0')
{
c.showError('\u9A8C\u8BC1\u5931\u8D25\uFF0C\u8BF7\u4F7F\u7528\u5F53\u524D\u90AE\u7BB1\u5173\u8054\u7684QQ\u53F7\uFF08'+c.getUin()+'\uFF09\u91CD\u65B0\u9A8C\u8BC1');
a._oDialog.close();
return;
}
...
a.doDefaultAction(g,f);
}
};
}
getTop().extend(toVerifyQQAccount,_oOidbAccountBase,{_oTmpl:getTop().TE(['<iframe frameborder="0" width="100%" height="336" src="$ptloginurl$" id="importFrame" allowtransparency="true"></iframe>']),_PTLOGIN_TEMP:getTop().T(["https://xui.ptlogin2.qq.com/cgi-bin/xlogin?appid=522005705&daid=4&s_url=$topHost$/cgi-bin/pt_check?action=checkskey%26","sid=$sid$%26t=pt_check%26cbName=$cbName$&style=25&low_login=0&proxy_url=$topProtocol$","//mail.qq.com/proxy.html&need_qr=0&hide_border=1&border_radius=0&self_regurl=http://zc.qq.com/chs/index.html?","type=1&app_id=11005?t=regist&pt_feedback_link=http://support.qq.com/discuss/350_1.shtml&css=https://","res.mail.qq.com/zh_CN/htmledition/style/$cssFilename$$ver$.css&target=self&hide_title_bar","=1&default_uin=$uin$&pt_no_auth=1&login_text=$loginDesc$&enable_qlogin=0"]),getCssVer:function(){
if(!this._msVer)
{
var a=getTop().getRes("$css_path$ptlogin_input_for_second_login44a10d.css"),b=(/ptlogin_input_for_second_login(.*)\.css/.test(a)&&RegExp.$1)||"";
this._msVer=b;
}
return this._msVer;
},getDefaultCfg:function(){
...
if(!a._oDefaultCfg)
{
a._oDefaultCfg={"topHost":_oTop.getTopHost(),"sid":_oTop.getSid(),"topProtocol":_oTop.location.protocol,"ver":a.getCssVer(),"loginDesc":encodeURIComponent('\u9A8C\u8BC1'),"cbName":"toVerifyQQAccount","cssFilename":"ptlogin_input_for_second_login"};
}
return a._oDefaultCfg;
},ptlogin2OnResize:function(a){
return function(d,c){
a._moConfig.nWidth=(d+'px');
a.dfConfig_(a._moConfig);
var b=a.S("importFrame");
if(b)
{
b.style.height=c+'px';
}
...
var c=getTop(),b=this;
b._oDialog.close();
var d=c.getMainWin()||c;
c.rdVer&&c.rdVer("BaseVer",-1);
var a=c.QMCache._moCacheSet;
if(a&&a.readmail)
{
delete a.readmail;
}
d.location.href=('//'+d.location.host+d.location.pathname+d.location.search+'&r='+Math.random());
}});
var cardModal={openCardEditModal:function(){
var c=cardInfo.info;
var d=['<section class="modal_container_body" id="modal_container_body">','<div class="modal_container_body_main_info">','<div class="photo">','<img src="$imgUrl$">','</div>','<div class="name">','<input value="$name$" id="profileName">','<span>\u4F60\u53D1\u51FA\u7684\u6240\u6709\u90AE\u4EF6\uFF0C\u53D1\u4EF6\u4EBA\u5C06\u663E\u793A\u4F60\u7684\u90AE\u7BB1\u6635\u79F0</span>','</div>','</div>','<div class="modal_line"></div>','<div class="modal_container_body_other_info">','<p class="title">\u8865\u5145\u4FE1\u606F</p>'];
while(c.otherInfos.length<2)
{
c.otherInfos.push({value:''});
}
for(var b=0;b<c.otherInfos.length;b++)
{
if(b===0)
{
d.push('<input placeholder="\u516C\u53F8/\u5B66\u6821/\u6240\u5C5E\u673A\u6784" value='+'"'+c.otherInfos[b].value+'"'+'/>');
continue;
}
if(b===1)
{
d.push('<input placeholder="\u804C\u52A1/\u5C97\u4F4D" value='+'"'+c.otherInfos[b].value+'"'+'/>');
continue;
}
if(b===2)
{
d.push('<input placeholder="\u624B\u673A/\u56FA\u8BDD/\u4F20\u771F" value='+'"'+c.otherInfos[b].value+'"'+'/>');
continue;
}
if(b===3)
{
d.push('<input placeholder="\u5730\u5740\u6216\u5176\u4ED6\u4FE1\u606F" value='+'"'+c.otherInfos[b].value+'"'+'/>');
continue;
}
}
d.push('<span class="icon" id="addOtherInfo"/>','</div>','</section>','<section class="modal_container_footer">','<div id="cancle">\u53D6\u6D88</div>','<div id="confirm">\u786E\u5B9A</div>','</section>');
var a=this;
this.qmDialog=new (getTop().QMDialog)({sId:"editCardInfo",sTitle:'\u7F16\u8F91\u9ED8\u8BA4\u5E10\u6237\u4FE1\u606F',nWidth:500,onshow:function(){
},sBodyHtml:T(d).replace({imgUrl:c.imgUrl,name:c.name}),onbeforeclose:function(){
},onload:function(){
var e=this;
var f=getTop();
f.addEvent(e.S('confirm'),'click',function(){
a.handleConfirm.call(e);
});
f.addEvent(e.S('cancle'),'click',function(){
e.close();
});
f.addEvent(e.S('addOtherInfo'),'click',function(){
a.addOtherInfo.call(this);
});
if(c.otherInfos.length>=4)
{
f.removeSelf(e.S('addOtherInfo'));
}
},onshow:function(){
this.S('profileName').focus();
}});
},addOtherInfo:function(){
var c=this.parentNode.children.length-2;
var d='';
if(c===0)
{
d='\u516C\u53F8/\u5B66\u6821/\u6240\u5C5E\u673A\u6784';
...
var a=this;
var b=a.S('modal_container_body');
var f=b.getElementsByTagName('input');
var h=f[0].value;
var c={mail:cardInfo.info.email,icon:cardInfo.info.imgUrl,name:h,other_infos:[],is_default_sign:cardInfo.profile.is_default_sign};
...
if(!/^\s*$/.test(k))
{
c.other_infos.push({value:k});
}
}
if(c.name.length===0)
{
getTop().showError('\u5E10\u53F7\u6635\u79F0\u4E0D\u80FD\u4E3A\u7A7A');
return;
}
else if(c.name.length>16)
{
getTop().showError('\u5E10\u53F7\u6635\u79F0\u4E0D\u80FD\u8D85\u8FC716\u5B57\u7B26');
return;
}
else if(/^[\d|\.]*$/.test(c.name))
{
getTop().showError('\u5E10\u53F7\u6635\u79F0\u4E0D\u80FD\u5168\u4E3A\u6570\u5B57');
return;
}
else{
for(var g=0;g<c.other_infos.length;g++)
{
if(c.other_infos[g].value.length>30)
{
getTop().showError('\u81EA\u5B9A\u4E49\u4FE1\u606F\u957F\u5EA6\u4E0D\u53EF\u8D85\u8FC730\u4E2A\u5B57\u7B26');
return;
}
}
}
cardInfo.setProfile.call(a,c);
}};
var cardInfo={imgNode:null,mainNode:null,otherNode:null,profile:{},info:{imgUrl:'',name:'',email:'',otherInfos:[]},init:function(){
this.imgNode=_oTop.S('settingDiv_Card_img_avatar',document);
this.mainNode=_oTop.CN('settingDiv_Card_info_item',document);
this.otherNode=_oTop.S('settingDiv_Card_other',document);
this.getProfile();
},set:function(f){
var b=f.alias_profile;
var d=f.main_profile;
var e={};
if(b)
{
var a=_oTop.getUserInfo("addr")||_oTop.getCookie("qqmail_alias")||'';
...
this.profile=e;
this.info.imgUrl=e.icon;
this.info.name=e.name;
this.info.email=e.mail;
this.info.otherInfos=e.other_infos||[];
this.updateCardView();
},updateCardView:function(){
var b=this.info;
this.imgNode.src=b.imgUrl;
this.mainNode[0].innerHTML=b.name;
this.mainNode[1].innerHTML=b.email;
var c='';
for(var a=0;a<b.otherInfos.length;a++)
{
c+='<p class="one_line_text">'+b.otherInfos[a].value+'</p>';
}
this.otherNode.innerHTML=c;
this.isDefaultSignNode.checked=this.profile.is_default_sign;
},selectCheck:function(c,a){
var b=c.checked;
this.setDefaultSignature(b);
},fillData:function(b,a){
S(b).innerHTML=a;
S(b).addEventListener('click',function(c){
c.preventDefault();
c.returnValue=false;
},true);
},getProfile:function(){
var a="/cgi-bin/xmail?xmmodule=addr&xmcgi=get_profile";
QMAjax.send(a,{method:'post',content:'func=1',onload:function(b,c){
if(b)
{
cardInfo.set(JSON.parse(c).body.profiles);
}
}});
},setProfile:function(c){
var b=this;
var a="/cgi-bin/xmail?xmmodule=addr&xmcgi=set_profile";
QMAjax.send(a,{method:'post',content:"profile="+encodeURIComponent(JSON.stringify(c)),headers:{},onload:function(d,e){
if(d)
{
getTop().showInfo('\u4FEE\u6539\u6210\u529F');
b.close();
cardInfo.getProfile();
}
else{
getTop().showError('\u4FEE\u6539\u5931\u8D25');
}
}});
},setDefaultSignature:function(b){
var a="/cgi-bin/xmail?xmmodule=addr&xmcgi=set_profile";
var e,d;
if(b)
{
...
var c=this.profile;
c.is_default_sign=b;
QMAjax.send(a,{method:'post',content:"profile="+encodeURIComponent(JSON.stringify(c)),headers:{},onload:function(f,g){
if(f)
{
getTop().showInfo(e);
cardInfo.getProfile();
...
var a={'comm2010.css':'$css_path$comm2010514351.css'};
return function(b){
var c=a[b];
return c&&getRes(c);
};
...
function getCssRules(l)
{
var c;
var f=/^https?:\/\/(.*?)([\/?].*?)*$/.exec(l.location.href);
if(f&&f.length>1)
{
c=f[1];
}
...
if(!g.href||c&&new RegExp('^https?://'+c).test(g.href))
{
var b=g.cssRules||g.rules;
if(b&&b.length)
{
for(var e=0;e<b.length;e++)
{
if(b[e].selectorText==='.skinlogo img')
{
if(a)
a+=';';
a+=d+'-'+b[e].cssText;
...
debug('getCssRules, domain: '+c);
debug('getCssRules, styleSheetReportStr: '+h);
debug('getCssRules, cssReportStr: '+a);
return {styleSheet:h,css:a};
}
function reportSkinLoad()
{
function b(d)
{
debug('report skin err! '+d);
xmailReport({itemName:'skin_report_err',expand:d.toString().replace(/,/g,'#_#'),logId:20376});
}
try{
var c=getCssRules(getTop());
waitFor(function(){
return document.querySelector('.skinlogo img');
},function(){
try{
var g='';
var f=document.querySelector('.skinlogo img');
if(f)
{
var i=/logo_(\d+)_[\d\w]+?\.(gif|png)/;
var h=i.exec(getComputedStyle(f).backgroundImage);
if(h&&h.length>1)
{
g=h[1];
}
}
var j=getTop().getPath('skin');
debug('skin: '+j);
debug('logo: '+g);
if(!getTop().S('report_skin_iframe'))
{
createIframe(getTop(),'https://rl.mail.qq.com/zh_CN/htmledition/ajax_proxy.html',{id:'report_skin_iframe',onload:function(){
try{
var l=this;
var m=l.contentDocument.createElement('link');
m.rel='stylesheet';
m.type='text/css';
m.href=getTop().style_path+'skin';
m.onload=function(){
try{
var o=getCssRules(l.contentWindow);
var p=[j,g,c.styleSheet,c.css,o.styleSheet,o.css,new Date()].join('##');
p=p?p.replace(/,/g,'#_#'):'';
if(g==='0'&&j!==g)
{
xmailReport({itemName:'skin_fail',expand:p,logId:20376});
}
}
catch(n)
{
...
this.contentDocument.head.appendChild(m);
}
catch(k)
{
b(k);
...
var k=d;
var j=a.calcPos(k,'json');
d.style.minHeight=j.width/2*Math.sin(c(20))+'px';
j=a.calcPos(k,'json');
var g=2*Math.ceil(j.height/300);
var h=[];
for(var m=0,n=g;m<n;m++)
{
h.push(m+1);
}
var o=j.width/4*Math.sin(c(15))-100;
var l=a.TE(['<div class="qm_watermark" style="width:$width$px;height:$height$px;">','$@$for($oLis$)$@$','<li class="qm_watermark_item" style="-webkit-transform:  translate(2%, ',o,'px) rotate(-15deg); -ms-transform: translate(2%, ',o,'px) rotate(-15deg);transform: translate(2%, ',o,'px) rotate(-15deg); ">$_root_.nickName$</li>','$@$endfor$@$','</div>']).replace({width:j.width,height:j.height,oLis:h,nickName:a.htmlEncode(e||a.getUserInfoText("alias")||a.getUin())});
a.insertHTML(k,"beforeEnd",l);
var f=function(){
var i=a.finds('div.qm_watermark',d);
if(i&&i.length>0)
{
i=i[0];
a.removeSelf(i);
}
a.removeEvent(a.getMainWin(),'resize',f);
a.removeEvent(a.getMainWin(),'load',f);
b(d,e);
};
a.addEvent(a.getMainWin(),'resize',f);
a.addEvent(a.getMainWin(),'load',f);
}
a.autoWaterMark=b;
})();
(function(a){
function e(h,j,i)
{
getTop().confirmBox({id:"meeting-reply-dialog",msg:'<div class="qui_txtDef">\u9700\u8981\u53D1\u9001\u7B54\u590D\u56DE\u6267\u7ED9\u53D1\u8D77\u4EBA\u5417\uFF1F</div>',title:"\u662F\u5426\u53D1\u9001\u7B54\u590D\u56DE\u6267",confirmClass:"qui_btn_Blue",onreturn:i,confirmBtnTxt:'\u53D1\u9001',cancelBtnTxt:'\u4E0D\u53D1\u9001'});
}
function f(l,k,n,i,h,m,j)
{
doXmailRequest('/calendar/reply',{cal_id:{relative_id:l,id:i,uid:n},attendee:{name:getTop().nn,email:k,status:h},calendar:j,no_response:!m});
}
function c(h)
{
xmailReport({itemName:h,logId:16997});
}
a.reportIcs=c;
function b(h,l,k,j,m,i)
{
e(h,l,function(p){
f(k,j,l.uid,l.id||'',h,p,i);
var r=m[0],q=m[1],n=m[2],o=m[3];
r.style.display='none';
if(h===2)
{
q.style.display='inline-block';
c('Meeting_web_read_invitation_tbd_click');
}
if(h===4)
{
o.style.display='inline-block';
c('Meeting_web_read_invitation_deny_click');
}
if(h===3)
{
n.style.display='inline-block';
c('Meeting_web_read_invitation_accept_click');
}
c('Meeting_web_read_invitation_reply_window_expose');
if(p)
c('Meeting_web_read_invitation_reply_yes_click');
else c('Meeting_web_read_invitation_reply_no_click');
});
}
a.replyInvitation=b;
function g(s,r)
{
var l=new Date(s*1000),k=l.getFullYear(),j=l.getMonth()+1,i=l.getDate(),n=l.getHours(),p=l.getMinutes();
var t='\uFF08\u661F\u671F'+"\u65E5\u4E00\u4E8C\u4E09\u56DB\u4E94\u516D".charAt(l.getDay())+'\uFF09';
var o=new Date(s*1000).toDateString()===new Date().toDateString();
var q=r?'':(o?'\u4ECA\u5929':k+'\u5E74'+j+'\u6708'+i+'\u65E5'+t);
return q+(n<12?'\u4E0A\u5348':'\u4E0B\u5348')+(n<=12?n:n-12)+' : '+(p<10?'0'+p:p);
}
function d(j,h,i)
{
if(!j||!h||!i)
...
doXmailRequest('calendar/parser',{relative_id:j,vcalendar:h},function(n,k){
if(n&&k&&k.schedule)
{
var m=k.schedule;
var o=m.subject.length<=40?m.subject:m.subject.substring(0,40)+'\u2026';
var l=g(m.start_time)+' - '+g(m.end_time,new Date(m.start_time*1000).toDateString()===new Date(m.end_time*1000).toDateString());
if(m.location)
{
l+=", "+(m.location.length<=40?m.location:m.location.substring(0,40)+'\u2026');
}
i(o,l,m);
}
});
